...
export type SchematicType = 'react' | 'workspace';
...
export default class LibraryBuilder {
  ...
  buildExecString(options: UserResponses, isDryRun: boolean): string {
    let commandBuilder: string[] = [
      `nx g @nrwl/${options.schematicType}:library`
    ];

    if (options.domain === 'shared' && options.libraryType !== 'util') {
  ...
  executeAddLibSchematic(options: UserResponses, isDryRun: boolean) {
    spawn(this.buildExecString(options, isDryRun), null, {
      stdio: 'inherit',
      shell: true
    });
  }
  ...
  async promptUser() {
    ...
    console.log('Here is a preview of the lib creation:');
    this.executeAddLibSchematic(userResponses, true);

    setTimeout(async () => {
      console.log('');
      const canProceedAndBuild = await this.getUserToggleResponse(
        'Does this look good? If yes, we can build the new lib.'
      );

      if (canProceedAndBuild) {
        this.executeAddLibSchematic(userResponses, false);
      }
    }, 4000);
  }
}
